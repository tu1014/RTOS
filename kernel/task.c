...
/*static KernelTcb_t* Scheduler_priority_algorithm(void) {
    ...
    for(uint32_t i = 0; i < sAllocated_tcb_index; i++) {
        ...
        if(pNextTcb != pNextTcb) {
            if(pNextTcb->priority <= sTask_list[sCurrent_tcb_index].priority) return pNextTcb;
        }
...
static __attribute__ ((naked)) void Save_context(void) {
    ...
    // save current tassk context into the current task stack
    __asm__ ("PUSH {lr}");
    ...
    __asm__ ("LDR r0, [r0]");
    __asm__ ("STMIA r0!, {sp}"); // == sCurrent_tcb->sp = ARM_CORE_SP_VALUE
}
